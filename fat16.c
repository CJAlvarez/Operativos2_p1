...
#include "fat16.h"
unsigned short anterior_cluster;
unsigned int actual_cluster;
...
int fat16_CD(char *filename,char anterior){
    char bytes_read;
    unsigned int root_start;
    if(anterior == 'r'){
        anterior_cluster = 0x4C200;
    }
    if (strcmp("..",filename) == 0){
        if(anterior == 'r'){
            //fat16_init();
            printf("%04X",anterior_cluster);
            fat16_seek(anterior_cluster);
            bytes_read = fat16_read_file(sizeof(Fat16Entry));
            print_file_info();
            
        }else{
            fat16_state.cluster = actual_cluster;
            fat16_state.cluster_left = fat16_state.sectors_per_cluster * 512;
            fat16_seek(fat16_state.data_start + (fat16_state.cluster-2) * 
            fat16_state.sectors_per_cluster * 512);
            bytes_read = fat16_read_file(sizeof(Fat16Entry));
            bytes_read = fat16_read_file(sizeof(Fat16Entry));
            fat16_state.cluster = FAT16_entry->starting_cluster;
            actual_cluster = FAT16_entry->starting_cluster;
            fat16_seek(fat16_state.data_start + (fat16_state.cluster-2) * 
            fat16_state.sectors_per_cluster * 512);
        }
    }else{
        if(fat16_open_file(filename, "   ") == 0){
                actual_cluster = FAT16_entry->starting_cluster;
        }
    }
    return -1;
